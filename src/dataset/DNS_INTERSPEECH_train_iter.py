...
from src.util.acoustic_utils import norm_amplitude, tailor_dB_FS, is_clipped, load_wav, subsample, stft, mag_phase, \
    get_complex_ideal_ratio_mask
...
class MapDataset(BaseDataset):
    def __init__(self,
        ...
        self.crop_at_a_time_len = crop_at_a_time_len
    ...
    def __getitem__(self, item):
        ...
        clean_y_list = [subsample(clean_y, sub_sample_length=int(self.sub_sample_length * self.sr))
                   for _ in range(self.crop_at_a_time_len)]

        noise_y = self._select_noise_y(target_length=int(self.sub_sample_length * self.sr)*self.crop_at_a_time_len)
        noise_y_list = noise_y.reshape((self.crop_at_a_time_len, -1))

        assert len(clean_y_list[0])*len(clean_y_list) == len(noise_y), f"Inequality: {len(clean_y)} {len(noise_y)}"

        for i, (clean_y, noise_y) in enumerate(zip(clean_y_list, noise_y_list)):
            ...
            noise_y_list[i] = torch.tensor(noisy_y.astype(np.float32))
            clean_y_list[i] = torch.tensor(clean_y.astype(np.float32))

        return zip(clean_y_list, noise_y_list)
...
class Dataset(IterableDataset):
    def __init__(self,
        ...
        super().__init__()
        self.dataset = MapDataset(clean_dataset,
                                  clean_dataset_limit,
                                  clean_dataset_offset,
                                  noise_dataset,
                                  noise_dataset_limit,
    ...
    def __len__(self):
        return len(self.dataset)*self.crop_at_a_time_len

    def get_stream(self, indexes_to_map):
        for i in indexes_to_map:
            samples = self.dataset[i]
            yield from samples

    def __iter__(self):
        indexes_to_map = torch.randperm(len(self.dataset))
        return self.get_stream(indexes_to_map)
