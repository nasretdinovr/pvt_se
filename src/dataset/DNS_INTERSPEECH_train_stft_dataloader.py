...
class DatasetSTFT(Dataset):
    ...
    def __getitem__(self, item):
        noisy_y, clean_y = super().__getitem__(item)
        noisy_complex = self.torch_stft(noisy_y)
        clean_complex = self.torch_stft(clean_y)
        ground_truth_cIRM = get_complex_ideal_ratio_mask(noisy_complex, clean_complex)
        noisy_mag_phase = torch.stack(mag_phase(noisy_complex), dim=0)
        return noisy_mag_phase, ground_truth_cIRM
