...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        print(f"tran epoch {epoch} on rank {self.rank}")
        self.train_dataloader.dataset.set_folder(epoch)
        self.train_dataloader.sampler.dataset.set_folder(epoch)
        sampler = self.train_dataloader.sampler
        self.train_dataloader.sampler.num_samples = int(math.ceil(len(sampler.dataset) * 1.0 / sampler.num_replicas))
        self.train_dataloader.sampler.total_size = sampler.num_samples * sampler.num_replicas

        with tqdm(BackgroundGenerator(self.train_dataloader, max_prefetch=8), desc=desc, total=len(self.train_dataloader)) as pgbr:
    ...
    def _validation_epoch(self, epoch):
        ...
        for i, (noisy, clean, name, speech_type) in tqdm(enumerate(self.valid_dataloader), desc="Validation"):
            ...
            pred_cRM = self.model.module(noisy_complex.permute(0, 3, 1, 2).contiguous())
            pred_cRM = pred_cRM.permute(0, 2, 3, 1).contiguous()
            ...
            if item_idx_list[speech_type] <= visualization_n_samples:
                self.spec_audio_visualization(noisy, enhanced/np.max(np.abs(enhanced)), clean, name, epoch, mark=speech_type)
