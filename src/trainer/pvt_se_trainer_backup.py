...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        with tqdm(BackgroundGenerator(self.train_dataloader, max_prefetch=10), desc=desc, total=len(self.train_dataloader)) as pgbr:
            for noisy, clean in pgbr:
                ...
                loss = self.loss_function(pred_cRM, ground_truth_cIRM)
                ...
                grads = torch.mean(torch.abs(self.model.module.head.out_layer.conv.weight.grad)).item()
                pgbr.desc = desc + 'grads = {:7.5f} loss = {:5.3f}'.format(grads, loss.item())
