...
from src.util.acoustic_utils import stft, istft, transform_pesq_range
...
class BaseTrainer:
    def __init__(self,
        ...
        self.loss_function_mask = loss_function_mask
        self.loss_function_spec = loss_function_spec
        self.loss_function_signal = loss_function_signal
        ...
        power = self.acoustic_config["power"]
    ...
    def spec_audio_visualization(self, noisy, enhanced, clean, name, epoch, noise=None, despeeched=None, mark=""):
        ...
        if noise is not None:
            ...
            despeeched_mag, _ = librosa.magphase(
                self.librosa_stft(despeeched, n_fft=320, hop_length=160, win_length=320))
            mags = [noisy_mag, enhanced_mag, clean_mag, noise_mag, despeeched_mag]
        else:
            mags = [noisy_mag, enhanced_mag, clean_mag]

        fig, axes = plt.subplots(len(mags), 1, figsize=(6, 10))
