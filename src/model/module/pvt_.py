...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, spec_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.depths = depths
        # patch_embed
        self.patch_embed1 = PatchEmbed(spec_size=spec_size, patch_size=patch_size[0], in_chans=in_chans,
...
def pvt_medium(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        patch_size=[4, 2, 2, 2], embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
def pvt_freq(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        patch_size=[(8,2), (2,2), (2,1), (1,1)], embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
if __name__ == '__main__':
    ...
    model = pvt_freq(spec_size=torch.tensor([f, t]), in_chans=12)
    model.to(1)
    output = model(inp)
