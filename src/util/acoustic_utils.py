...
def get_complex_ideal_ratio_mask(noisy_complex_tensor, clean_complex_tensor, compress=True):
...
def get_dct_ideal_ratio_mask(noisy_dct_tensor, clean_dct_tensor, compress=True):

    ideal_ratio_mask = clean_dct_tensor / (noisy_dct_tensor + 1e-10)
    if compress:
        ideal_ratio_mask = compression_using_hyperbolic_tangent(ideal_ratio_mask, K=10, C=0.1)

    return ideal_ratio_mask
...
#
#
#     return librosa.stft(y, n_fft, hop_length, win_length, window, center, dtype, pad_mode)
#
#
# def librosa_istft(y,
#                   hop_length=None,
...
#                   power=0.3):
#
#     y = librosa.istft(y, hop_length, win_length, window, center, dtype, length)
#     return y
